...
const TEMPLATES: CategoryTemplate[] = [
  ...
  {
    category: '测试',
    iconClassName: 'bg-linear-to-br from-emerald-500 to-teal-600',
    entries: [
  ...
  {
    category: '写作',
    iconClassName: 'bg-linear-to-br from-violet-500 to-purple-600',
    entries: [
  ...
  {
    category: '运维',
    iconClassName: 'bg-linear-to-br from-slate-500 to-slate-700',
    entries: [
...
// 以 id 为键的 O(1) 索引，替代对 SKILLS 的 O(N) find。
const SKILL_BY_ID = new Map<string, Skill>(SKILLS.map((s) => [s.id, s]));

export const getSkillById = (id: string): Skill | undefined => SKILL_BY_ID.get(id);
...
export const AGENTS: Array<'全部' | SkillAgent> = ['全部', 'Codex', 'Claude Code'];
